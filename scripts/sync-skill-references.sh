...
stage_concat() {
  ...
  mkdir -p "$(dirname "$out")"
  {
    echo "<!-- GENERATED by scripts/sync-skill-references.sh from: $* -->"
    echo "<!-- Do not hand-edit; edit the source files above and re-run the sync script. -->"
    echo
    echo "$header"
    local first=1
    for src in "$@"; do
...
# Concatenate several sources into one generated reference (round-trip
# reduction for a consolidated Task that would otherwise open every source
# file separately, e.g. Plan Slice reading judge+apply-api+apply-dbc+derive-tests).
concat_one() {
  local dest="$1"
  local header="$2"
  shift 2
  ...
  if [[ "$MODE" == "check" ]]; then
    if [[ ! -f "$dest" ]]; then
      echo "MISSING  $dest  (concat)"
      mismatch=1
      return
    fi
    if ! cmp -s "$staged" "$dest"; then
      echo "STALE    $dest  (concat)"
      mismatch=1
    else
      echo "ok       $dest"
    fi
...
# Plan Slice cheat sheet: consolidate judge + apply-api + apply-dbc + derive-tests
# (SKILL.md + their primary adapter reference) into one generated file so the
# Plan Slice Task (solidsdd-loop) can read one file instead of ~7 separate ones.
# GraphQL/RSpec adapters, the full application-plan schema, and human-gates.md
# stay as separate linked fallbacks (edge cases, not the common path) rather
# than being inlined here.
concat_one skills/solidsdd-loop/references/plan-slice-cheatsheet.md \
...
Read this ONE file for the Plan Slice Task instead of opening the four sibling skills' SKILL.md + primary adapter reference separately — it is a mechanical concatenation of their content (regenerated by \`scripts/sync-skill-references.sh\`, always in sync with the sources). Fall back to the full skill (via its own \`references/\`) for: GraphQL (see \`solidsdd-apply-api/references/graphql-adapter.md\`), Ruby/RSpec (see \`solidsdd-apply-dbc/references/ruby-rspec-adapter.md\` or \`solidsdd-derive-tests/references/ruby-rspec-adapter.md\`), exact ApplicationPlan JSON Schema validation (\`solidsdd-judge/references/application-plan.schema.json\`), or human-gate rules beyond the Combining rules below (\`solidsdd-judge/references/human-gates.md\`)." \
  skills/solidsdd-judge/SKILL.md \
  reference-src/judgment-axes.md \
  skills/solidsdd-apply-api/SKILL.md \
  adapters/openapi/README.md \
  skills/solidsdd-apply-dbc/SKILL.md \
  adapters/ocl/README.md \
  skills/solidsdd-derive-tests/SKILL.md
