...
- Use **exactly** the required top-level headings in [change-context.md](references/change-context.md)
...
2. If only legacy flat Brief exists, migrate per [change-lifecycle.md](references/change-lifecycle.md) first when paths collide; otherwise proceed.
3. Decide `change_id`: use caller-supplied id when valid; otherwise derive meaningful kebab-case from goal; on collision under `changes/`, append `-2`, `-3`, …
4. If another change is `active` and this is a **new** change, set the previous `status.json` to `done` or `abandoned` as appropriate before switching.
5. Apply section rules in [change-context.md](references/change-context.md). Especially:
...
6. Decide Change Context gate per [change-context.md](references/change-context.md) §Human gate and [human-gates.md](references/human-gates.md):
...
7. Validate gate JSON against [change-context-gate.schema.json](references/change-context-gate.schema.json).
8. Create `.solidsdd/changes/<change_id>/` and write:
   - `change-context.md`
...
The Markdown path, gate path, `change_id`, whether `human_gate.required`, and a one-paragraph summary. Return these to the parent unchanged in meaning.
