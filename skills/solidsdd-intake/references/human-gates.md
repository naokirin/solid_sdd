...
| Architecture (structural) | Large module/boundary change; a new **external-facing** boundary; a reversal of an existing dependency direction; a deliberate removal/change of an existing structural `constraint` — see [architecture-axes.md](architecture-axes.md). **Do not** gate ordinary additive module/dependency additions. A project's **first-ever** Architecture Model (`workspace.dsl` didn't exist before this change) counts as a large/external-facing boundary change by default — gate it, unless the model is genuinely small and entirely internal (no element the change intends as an external-facing entry point). **Owner: `solidsdd-architecture` only** — it sets this on the generated `architecture-plan.json`'s `human_gate` when it writes `status: changed`. Any other skill that only *reads* ArchitecturePlan as context (e.g. `solidsdd-judge`'s `architecture_public_boundary` signal) must not re-apply or duplicate this row — the gate, if warranted, is already recorded upstream |
