...
2. **Triage or resume** (parent, no Task):
   ...
   - Else (new change): run **Triage** now. Get `requested_profile` via `scripts/solidsdd-next.sh parse-profile --text "<the user's instruction>"` ([solidsdd-next.md](references/solidsdd-next.md)) — prefer this mechanical extraction over ad hoc prose parsing of `--profile <x>` / `profile: <x>`; default `auto` when it reports no explicit token (and note any `warning` it returns, e.g. a near-miss token, in the final summary). Evaluate change type / complexity / risk / contract impact / architecture impact per [triage.md](references/triage.md); compute `required_minimum_profile` and `effective_profile = max(requested, required_minimum)`. Write `.solidsdd/changes/<change_id>/triage-result.json` ([triage-result.schema.json](references/triage-result.schema.json)), creating the change dir per [change-lifecycle.md](references/change-lifecycle.md) if needed (pass optional user `change_id`).
...
2. Optional: write a short Context/Impact note at the canonical change-level **context pack** path, `.solidsdd/changes/<change_id>/context-pack-framing.md` ([Context pack](references/execution-model.md#context-pack) — same path/shape used for outer Intake/Brief Tasks, not a separate ad hoc file), when it will materially help the Task prompts: a few lines on touched files and why this is scoped thin. This is **not** a ChangeBrief — do not give it `human_gate` / `success_criteria` / scope fields. Writing it at this path (rather than a one-off filename) is what lets **Escalation** below hand it straight to Intake/Brief without a conversion step.
...
4. **From Thin (L1)**: keep the existing reduced `run-state.json`'s `triage` / `thin_implementation` / `thin_verification` history; initialize the remaining Standard/Full phases the new profile requires (usually starting at `intake`). If a Context/Impact note was written, it already lives at `context-pack-framing.md` (see step 2 above) — **append** the L1 attempt's outcome (touched files, a short diff summary, the `verification-report.json` path/result) to that same file, then pass its path into the Intake/Brief Task prompt as prior framing; no separate note or conversion step ([Context pack — escalation carry-forward](references/execution-model.md#context-pack)). Do not redo the L1 implementation from scratch — but do run every Standard/Full checkpoint (Brief, WorkPlan, Architecture Judgment, and their critiques) that the new profile requires and L1 skipped; escalation upgrades assurance, it does not retroactively grant it. When `solidsdd-decompose` and `solidsdd-loop` later produce the WorkPlan item(s) superseding this attempt, that item's own `items/<item_id>/context-pack.md` should cite `context-pack-framing.md`'s escalation record under **Precedent to adapt** rather than re-deriving what the L1 attempt already discovered.
...
**Thin (L1):** `triage`, `[context-pack-framing.md note if written]`, `implement` (Task), `verify` (Task), `[critique if failure/uncertainty triggered escalation]` — and explicitly confirm brief/decompose/architecture/waves/knowledge-harvest did **not** run.
