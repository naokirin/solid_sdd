...
| 2 | Logical decomposition | Responsibility/boundary/ownership genuinely needs re-deciding (e.g. a module is doing two unrelated things) | Extract Decision Drivers when the decision needs justification beyond the trigger, then work through Logical Decomposition (Responsibility / State Ownership / Knowledge Ownership / Change Locality, plus Consistency Boundary / Concurrency Boundary when this change hinges on either — see [architecture-axes.md](architecture-axes.md)) before touching the DSL |
| 3 | Physical decomposition | The logical split from Level 2 needs a concrete new module/package/component, not just a documented boundary | Level 2, then also add the new `container`/`component` element(s) in `workspace.dsl`; when the physical realization itself is non-obvious, also write [Physical Design](physical-design.md) — do not let directory structure drive the logical decision |
| 4 | Formal architectural reasoning | Concurrency, distributed state, complex state transitions, race conditions, ordering guarantees, safety/liveness concerns | Levels 2–3 as needed — a Concurrency/Consistency Boundary at this level often needs Level 3's process/service/database allocation *decided*, not just documented (see [Physical Design](physical-design.md) "When to write this") — then hand off to TLA+/Alloy (`solidsdd.judge` / `ApplicationPlan`) for the state/temporal properties. Architecture states *who owns the state and where the boundary is*, not the transition semantics |
