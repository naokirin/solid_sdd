...
1. Read ChangeBrief (`.solidsdd/change-brief.json` when present), change intent, and context (`solidsdd-context` output if available).
2. If input is prose or incomplete Gherkin, normalize to Feature/Scenario form per [gherkin-requirements.md](references/gherkin-requirements.md) (create/update `.feature` under default layout when useful).
