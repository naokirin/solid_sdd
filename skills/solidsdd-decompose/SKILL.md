...
2. If input is prose or incomplete Gherkin, normalize to Feature/Scenario form per [gherkin-requirements.md](references/gherkin-requirements.md) (create/update `.feature` under default layout when useful; new Scenarios for this Brief only, with coverage tags). For each operation-shaped Scenario, apply gherkin-requirements.md convention 4's failure-path checklist (references an existing entity by id? transitions state? has a capacity/uniqueness constraint?) and add the matching failure Scenario(s) now — do not leave that discovery for a later `solidsdd-judge` / `solidsdd-apply-dbc` / `solidsdd-apply-api` pass.
