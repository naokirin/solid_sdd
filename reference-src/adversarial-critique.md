...
| Failure-path traceability gap | A contract (OCL `pre`, OpenAPI error response, etc.) formalizes a named failure mode (e.g. "already claimed", "not found") that has **no** corresponding Gherkin Scenario anywhere in scope stating that failure as a requirement — i.e. the contract invented behavior the requirements layer never asked for, backwards from [gherkin-requirements.md](gherkin-requirements.md) convention "name failure paths explicitly" | The failure mode is only an internal/defensive check with no product-level meaning (e.g. a type-echo null guard); the failure is already stated as part of a sibling Scenario's `Then`/`And` steps (even inside a larger scenario, e.g. the losing side of a concurrency Scenario) |
