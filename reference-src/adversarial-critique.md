...
Before LLM review, run **`scripts/solidsdd-lint.sh`** against the consuming project (documented in solid_sdd `scripts/solidsdd-lint/README.md`; skill copy: [solidsdd-lint.md](solidsdd-lint.md) when installed via sync). Import lint findings into the CritiqueReport (`blocker`/`major`/`minor` as emitted) **that are relevant to the requested `subject`** — lint validates the whole change directory unconditionally in one pass (Change Context / Brief / WorkPlan / ArchitecturePlan / NFR / gate files all at once, regardless of what you were asked to review), so a run will routinely surface findings that belong to a *different* checkpoint than the one you're evaluating (e.g. a `change_brief`/`work_plan` schema finding when your `subject` is `architecture_plan`). Match a finding's `location` against the Inputs listed for your `subject` in the table above; only in-scope findings count toward this report's `result`/`findings`. Still mention out-of-scope findings you observed in the report's `summary` (so nothing silently disappears), but do not let them fail this subject's review or point `loop_action`/`suggested_next_skills` at this subject's producer. Lint owns: schema shape, `change_id` match, `depends_on` cycles, Gherkin shape, Brief id coverage via `covers` / Scenario tags, ambiguity lexicon hits.
...
| Failure-path / concurrency-scenario traceability gap | A contract (OCL `pre`, OpenAPI error response, etc.) or formal spec formalizes a named failure mode or safety property (e.g. "already claimed", "not found", "at most one claimant") that has **no** corresponding Gherkin Scenario anywhere in scope stating it as a requirement — i.e. the contract/formal layer invented behavior the requirements layer never asked for, backwards from [gherkin-requirements.md](gherkin-requirements.md) convention 4. This includes: a `success_criteria` states an invariant across simultaneous actors ("exactly one wins") but Gherkin has only a sequential already-in-state Scenario and no Scenario dramatizing concurrent/simultaneous attempts | The failure mode is only an internal/defensive check with no product-level meaning (e.g. a type-echo null guard); the failure/property is already stated as part of a sibling Scenario's `Then`/`And` steps (even inside a larger scenario) |
...
| Physical Design missing/hollow | [Architecture Depth](architecture-depth.md) is clearly Level 3 (WorkPlan `touches` implies a genuinely non-obvious realization — see [architecture-traceability.md](architecture-traceability.md) "When explicit traceability is required" triggers, also linked from [physical-design.md](physical-design.md) "When to write this") but `physical-design.md` is absent, or present but only restates the Logical structure without naming the physical/allocation decision | Level 3 with a `physical-design.md` that names the actual boundary-enforcement mechanism; or Level 3 with a trivial 1:1 mapping correctly left unwritten **and no unresolved process/service/database boundary — see the next row** |
| Physical Design skipped despite unresolved allocation | `architecture-reasoning.md` records a Consistency Boundary or Concurrency Boundary (a component coordinating across concurrent/distributed callers) but `physical-design.md` is absent and neither `architecture-reasoning.md` nor `workspace.dsl` states how that coordination is physically enforced (single process? shared database? distributed lock?) — a trivial 1:1 file-path mapping does not resolve this; do not accept "every module maps to one file" as evidence Physical Design was unneeded when this applies | The Consistency/Concurrency Boundary is clearly single-process / no cross-process concern (e.g. an in-memory validation rule with no stated multi-process/multi-machine callers); or `physical-design.md` does name the process/service/database allocation |
