...
- [x] `scripts/solidsdd-next/next.py parse-profile --text "<instruction>"`: mechanical `--profile <x>` / `profile: <x>` extraction for Triage (case-insensitive; warns and defaults to `auto` on a near-miss token like `--profile fast`) instead of ad hoc prose parsing
- [x] Thin (L1)'s optional Context/Impact note is written directly at the canonical `context-pack-framing.md` path (not a separate ad hoc file); on escalation the parent appends the L1 attempt's outcome to that same file and hands it straight to Intake/Brief, and the superseding WorkPlan item's `context-pack.md` cites it under Precedent to adapt — see [execution-model.md](execution-model.md) "Context pack"
